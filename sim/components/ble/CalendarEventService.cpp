...
//                         .value = {0xd0, 0x42, 0x19, 0x3a, 0x3b, 0x43, 0x23, 0x8e, 0xfe,
//                         0x48, 0xfc, 0x78, y, x, 0x05, 0x00}};
// }
...
int dummy_callback(uint16_t                     conn_handle,
...
CalendarEventService::CalendarEventService(System::SystemTask& systemTask,
                                           DateTime&           dateTimeController)
  : systemTask{systemTask}, dateTimeController{dateTimeController}
{
...
int CalendarEventService::OnAddCalendarEvent(uint16_t                     conn_handle,
                                             uint16_t                     attr_handle,
                                             struct ble_gatt_access_ctxt* ctxt)
{
  ClearOldEvents();
  ...
  //   const auto strsSize =
  //     std::min(CalendarEvent::maxStringsSize, packetLen - CalendarEvent::headerSize);

  //   decltype(CalendarEvent::id) newEvId{};
  //   int                         res =
  //     os_mbuf_copydata(ctxt->om, CalendarEvent::idOffset, sizeof(newEvId), &newEvId);
  //   ASSERT(res == 0);
  //   NRF_LOG_INFO("Cal ev id: %ld", newEvId);
  //   // remove existing event with the same id for replacement
  //   {
  //     auto it =
  //       std::find_if(std::begin(calEvents),
  //                    std::end(calEvents),
  //                    [newEvId](CalendarEvent const& ev) { return ev.id == newEvId; });
  ...
  //   res = os_mbuf_copydata(ctxt->om,
  //                          CalendarEvent::timestampOffset,
  //                          sizeof(newEvTimestamp),
  //                          &newEvTimestamp);
  //   ASSERT(res == 0);
  ...
  //     newEv->id = newEvId;
  //     res = os_mbuf_copydata(ctxt->om,
  //                            CalendarEvent::durationOffset,
  //                            sizeof(newEv->durationInSeconds),
  //                            &newEv->durationInSeconds);
  //     ASSERT(res == 0);
  //     newEv->timestamp = newEvTimestamp;
  //     res = os_mbuf_copydata(ctxt->om,
  //                            CalendarEvent::colorOffset,
  //                            sizeof(newEv->color),
  //                            &newEv->color);
  //     ASSERT(res == 0);
  //     res = os_mbuf_copydata(ctxt->om,
  //                            CalendarEvent::stringsOffset,
  //                            strsSize,
  //                            newEv->strings.data());
...
int CalendarEventService::OnRemoveCalendarEvent(uint16_t                     conn_handle,
                                                uint16_t                     attr_handle,
                                                struct ble_gatt_access_ctxt* ctxt)
{
  // decltype(CalendarEvent::id) idToRemove{};
  // int                         res =
  //   os_mbuf_copydata(ctxt->om, CalendarEvent::idOffset, sizeof(idToRemove),
  //   &idToRemove);
  // ASSERT(res == 0);
  // auto it =
  //   std::find_if(std::begin(calEvents),
  //                std::end(calEvents),
  //                [idToRemove](CalendarEvent const& ev) { return ev.id == idToRemove;
  //                });
  // if (it != std::end(calEvents))
  // {
...
CalendarEventService::EventRange::iterator
CalendarEventService::FindSpotForEvent(std::int32_t timestamp)
{
  return calEvents.emplace(std::find_if(std::begin(calEvents),
                                        std::end(calEvents),
                                        [timestamp](CalendarEvent const& ev)
...
void CalendarEventService::ClearOldEvents()
{
  ...
  {
    auto curIt = it++;
    if ((curIt->timestamp + curIt->durationInSeconds + keepTimeInSeconds) <
