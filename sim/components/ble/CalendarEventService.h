...
{
public:
  explicit CalendarEventService(System::SystemTask& systemTask,
                                DateTime&           dateTimeController);

  void Init();

  int OnAddCalendarEvent(uint16_t                     conn_handle,
                         uint16_t                     attr_handle,
                         struct ble_gatt_access_ctxt* ctxt);
  int OnRemoveCalendarEvent(uint16_t                     conn_handle,
                            uint16_t                     attr_handle,
                            struct ble_gatt_access_ctxt* ctxt);
  ...
  struct CalendarEvent
  {
    ...
    static constexpr std::size_t timestampOffset{durationOffset +
                                                 sizeof(durationInSeconds)};
    ...
    static constexpr std::size_t headerSize{sizeof(id) + sizeof(durationInSeconds) +
                                            sizeof(timestamp) + sizeof(color)};
